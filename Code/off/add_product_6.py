...
on remplace les ė par des é pour le cas Intermarché"""
store_set = set()
for product in products:
	store_list = (product['stores']).split(',')
	for store in store_list:
		store_set.add(((store.replace("ė", "é")).strip()).capitalize())
print(store_set)
...
for product in products:
	...
	for store in list_stores:
		print("Magasin =", (store.strip()).capitalize())
		cursor.execute(
			"""
			INSERT INTO
			...
			""",
			{"code": product['code'], "store_name": (store.strip()).capitalize()}	
		)
		cnx.commit()
